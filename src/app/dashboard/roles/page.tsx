...
export default function RolesPage() {
  ...
  const handleSaveRole = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const roleData = {
      ...
      permissions: {
        manage_users: e.currentTarget.manage_users.checked,
        manage_inventory: e.currentTarget.manage_inventory.checked,
        view_reports: e.currentTarget.view_reports.checked,
        manage_clients: e.currentTarget.manage_clients.checked,
        field_operations: e.currentTarget.field_operations.checked,
      }
    }
  ...
  const getUserCountByRole = (roleTitle: string) => {
    return users?.filter(u => u.roleId === roleTitle).length || 0
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {roles?.length === 0 && (
            <Card className="md:col-span-2 border-dashed border-2 py-20 flex flex-col items-center justify-center text-muted-foreground">
              ...
              <Button variant="link" onClick={() => setIsAdding(true)}>Cree el primer rol operativo</Button>
            </Card>
          )}
        </div>
      )}
