...
export default function CalendarPage() {
  ...
  const [linkedOS, setLinkedOS] = useState<any | null>(null)
  ...
  useEffect(() => {
    ...
    if (prefillOrderId) {
      const fetchOS = async () => {
        const docSnap = await getDoc(doc(db, "service_orders", prefillOrderId))
        if (docSnap.exists()) {
          setLinkedOS(docSnap.data())
        }
      }
      fetchOS()
    }
  }, [prefillClientId, prefillOrderId, db])
  ...
  const handleSaveAppointment = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const aptData = {
      ...
      orderId: prefillOrderId || editingApt?.orderId || null,
      orderNumber: linkedOS?.orderNumber || editingApt?.orderNumber || null
    }

    if (editingApt) {
      updateDocumentNonBlocking(doc(db, "appointments", editingApt.id), aptData)
      toast({ title: "Orden de Trabajo actualizada" })
    } else {
      ...
      toast({ title: "Orden de Trabajo programada" })
    }

    setIsAdding(false)
    setEditingApt(null)
    setLinkedOS(null)
    if (prefillClientId) router.replace("/dashboard/calendar")
  }

  const handleDelete = (id: string) => {
    if(!confirm("¿Anular esta Orden de Trabajo (OT)?")) return
    deleteDocumentNonBlocking(doc(db, "appointments", id))
    toast({ variant: "destructive", title: "OT cancelada" })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight mb-1 uppercase text-primary">Agenda de Órdenes de Trabajo (OT)</h2>
          <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">Programación técnica basada en pedidos de servicio.</p>
        </div>
        
        <Dialog open={isAdding} onOpenChange={(open) => { setIsAdding(open); if (!open) { setEditingApt(null); setLinkedOS(null); } }}>
            ...
            <form onSubmit={handleSaveAppointment} className="flex flex-col h-full overflow-hidden">
              <DialogHeader className="p-6 border-b bg-slate-50 shrink-0">
                <DialogTitle className="uppercase font-black text-primary">
                  {editingApt ? "Editar OT" : "Programar OT"}
                </DialogTitle>
                <DialogDescription className="text-[10px] font-bold uppercase">Asigne ejecución técnica para extintores.</DialogDescription>
              </DialogHeader>
              
              <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar min-h-0">
                {(prefillOrderId || editingApt?.orderNumber) && (
                  <div className="bg-accent/10 p-3 rounded-lg border border-accent/20 flex items-center gap-3">
                    <ClipboardList className="h-4 w-4 text-accent" />
                    <span className="text-[10px] font-black uppercase text-accent">Vinculado a OS: {linkedOS?.orderNumber || editingApt?.orderNumber}</span>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Estado de OT</Label>
              ...
              <DialogFooter className="p-6 border-t bg-slate-50 shrink-0">
                <Button type="submit" className="w-full uppercase font-black text-xs h-12 shadow-xl bg-primary text-white">
                  {editingApt ? "Guardar Cambios" : "Confirmar OT"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <Card className="lg:col-span-5 shadow-sm border-none bg-white">
          <CardHeader className="p-4 sm:p-6">
            <CardTitle className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Planificación Técnica</CardTitle>
          </CardHeader>
        ...
        <Card className="lg:col-span-7 shadow-sm border-none bg-white">
          <CardHeader className="border-b bg-slate-50/50 p-4 sm:p-6">
            <CardTitle className="text-[10px] sm:text-xs font-black uppercase flex items-center tracking-widest text-primary">
              ...
              OTs del día: {date ? format(date, "d 'de' MMMM, yyyy", { locale: es }).toUpperCase() : "---"}
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4 sm:p-6 space-y-4">
            {loadingApts ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-muted-foreground text-center opacity-40">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest">Sin trabajos programados</p>
              </div>
            ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {apt.orderNumber && <Badge className="bg-slate-100 text-slate-600 border-none text-[7px] font-black">OS: {apt.orderNumber}</Badge>}
                      </div>
                    ...
                    <div className="flex items-center gap-2 mt-2 sm:mt-0">
                      {apt.status !== "Completado" ? (
                        ...
                        <Badge className="bg-status-success text-white h-8 text-[8px] font-black px-4 uppercase flex items-center gap-1">
