...
/**
 * @fileOverview A Genkit flow for generating automated service reminders for extinguishers only.
 */
...
const AutomatedServiceReminderInputSchema = z.object({
  ...
  serviceHistory: z.array(
    z.object({
      ...
      type: z.enum(["extinguisher_maintenance", "extinguisher_inspection", "extinguisher_recharge", "extinguisher_rental", "extinguisher_sale"]).describe("The type of fire extinguisher service performed."),
...
const AutomatedServiceReminderOutputSchema = z.object({
  reminders: z.array(
    z.object({
      ...
      serviceType: z.enum(["extinguisher_maintenance", "extinguisher_inspection", "extinguisher_recharge", "extinguisher_rental", "extinguisher_sale"]).describe("The type of service."),
...
const automatedServiceReminderPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant for EXTINPRO, specializing ONLY in fire extinguisher management (NTP 350.043-1). Your goal is to analyze client extinguisher history and generate reminders for upcoming maintenance, inspection, or recharge.\n\nThe current date is {{{currentDate}}}.\n\nPrioritize reminders:\n- 'High' for services due within 7 days.\n- 'Medium' for 8-30 days.\n- 'Low' for 31-60 days.\n\nClient: {{{clientName}}}\nLocation: {{{serviceLocation}}}\n\nHistory:\n{{#each serviceHistory}}\n- Date: {{{date}}}\n  Type: {{{type}}}\n  Notes: {{{lastTechnicianNotes}}}\n  Next Due: {{{nextRecommendedDate}}}\n{{/each}}`
});
